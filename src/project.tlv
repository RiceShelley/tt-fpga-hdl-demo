...
\TLV calc()
   ...
   |calc
      @0
         $reset = *reset;
      @1
         //$a[7:0] = {3'b0, $rand1[4:0]};
         //$a[7:0] = $reset ? 0 : >>1$out[7:0];
         //$b[7:0] = {3'b0, $rand2[4:0]};
         
         $sel[1:0] = $uo_in[5:4];
         $b[7:0] = {4'b0, $uo_in[3:0]};
         $equals_in = $uo_in[7];
         
         $a[7:0] = $reset ? 0 : >>1$a + 1;
         //$sel[1:0] = $reset ? 0 : >>1$sel + 1;
         
         $valid_bits[1:0] = {>>1$equals_in, >>2$equals_in};
         $valid = $valid_bits == 2'b10;

         $sum[7:0] = $a[7:0] + $b[7:0];
         $diff[7:0] = $a[7:0] - $b[7:0];
         $prod[7:0] = $a[7:0] * $b[7:0];
         $quot[7:0] = $a[7:0] / $b[7:0];
